...
- **Acceptance criteria as checkable boxes.** Objectively true or false. Not
  "improve X".
...
If another agent or session might be working the repo, **make a worktree**:
`git worktree add <path> -b <branch>`. Check first with `git status --short` and
`git branch --show-current`. Uncommitted changes, or a branch you did not create,
mean someone else is in there. This applies to you, not only to agents you
dispatch. File ownership does not help: `checkout`/`pull` move the whole tree.
