...
| **require-code-review.py** | PreToolUse | Blocks `gh pr merge` on a 25+ line PR with no review, looked up in the repo the command targets (`cd`, `-R`, a PR URL, `GH_REPO`). A merge it can see and cannot read down to a literal PR and repo is blocked with a request for `-R owner/repo`. Override: `SKIP_REVIEW_GATE=1` as a prefix on the merge. |
