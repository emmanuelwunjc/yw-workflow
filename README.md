...
is written after its source line. The loop stops on a clean round. Each PR
body carries a `Rounds:` line with every round's verdict, blocking count and
token count, so rounds and tokens per PR are one command.
